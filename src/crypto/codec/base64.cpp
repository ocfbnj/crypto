...
namespace base64 {
namespace {
constexpr std::size_t adjust_multiple_of(std::size_t n, std::size_t num) {
...
std::vector<std::uint8_t> encode(std::span<const std::uint8_t> src) {
    const std::size_t n = (adjust_multiple_of(3, src.size()) * 4) / 3;
...
std::vector<std::uint8_t> decode(std::span<const std::uint8_t> src) {
    ...
    if (ret == MBEDTLS_ERR_BASE64_INVALID_CHARACTER) {
        throw crypto::codec::decoding_error{"base64 invalid character"};
    }
    assert(ret == 0);
